...
- Дополнительные параметры контекста передаются через замыкание, а не как аргументы `checkEvent`
...
fun createExpectedEvent(
    ...
    return ExpectedEvent(
        ...
        data = EventData(
            ...
            // дедлайн — считается на основе времени создания + конфигурации
...
|---|---|
| Строгое равенство | `status`, `entityId`, `type`, `subject` |
...
- **Передача неправильного контекста в `checkEvent`** — для каждого статуса передавай контекст, актуальный **на момент** этого статуса (например, для финального статуса — результат последнего шага, а не начальный контекст).
